...
{
	{
		if (!InitializeWindow({ 1280,720 },false,true,true))
			return -1;
		const auto& [renderPass, framebuffers] = vulkanLearning::CreateRpwf_Screen();
		...
		struct perFrameObjects_t {
			...
			//直接 = VK_FENCE_CREATE_SIGNALED_BIT也行，这里按照我的编程习惯在初始化类/结构体时保留花括号
		...
		while (!glfwWindowShouldClose(m_pWindow)) {
			...
			fence.WaitAndReset();
			//在渲染循环开头等待与当前交换链图像配套的栅栏（在渲染循环末尾等待的话，下一帧必须等待当前帧渲染完，即时帧的做法便没了意义）
